...
const SingleCard: React.FC<SingleCardProps> = ({ todo, todoList, setTodoList }) => {
    ...
    return (
        <form className="single__card" onSubmit={(e)=>handleEditTask(e,todo.id)}>
            {editMode?(
                <input ref={EditRef} value={editTodo} className="single__card--edit" onChange={(e)=>setEditTodo(e.target.value)}/>
            ):(
                todo.isDone?(<s className="single__card--title">{todo.todo}</s>):(<span className="single__card--title">{todo.todo}</span>)
            )}
